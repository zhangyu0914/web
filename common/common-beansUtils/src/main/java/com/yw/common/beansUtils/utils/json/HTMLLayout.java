...
public class HTMLLayout extends Layout
{
  ...
  public String format(LoggingEvent event)
  {
    ...
    try {
    	this.sbuf.append("<td>");
		this.sbuf.append(DateUtils.getTimestamp());//时间
    ...
    this.sbuf.append("<td title=\"" + escapedThread + " thread\">");//线程
    this.sbuf.append(escapedThread);
    this.sbuf.append("</td>" + Layout.LINE_SEP);

    this.sbuf.append("<td title=\"Level\">");//级别
    ...
    String ip = StringUtils.getNullToStr(MDC.get(LogEnum.IP.getCode()));
    this.sbuf.append("<td title=\"" + ip + "\">");//IP
    this.sbuf.append(ip);
    this.sbuf.append("</td>" + Layout.LINE_SEP);
    
      LocationInfo locInfo = event.getLocationInformation();
      this.sbuf.append("<td>");
      this.sbuf.append(Transform.escapeTags(locInfo.getClassName()));
      this.sbuf.append(':');
      this.sbuf.append(locInfo.getLineNumber());
      this.sbuf.append("</td>" + Layout.LINE_SEP);

    this.sbuf.append("<td title=\"Message\">");//信息
    ...
    try {
		LoggerUtil.pushLog(event.getRenderedMessage(), "webplatform", String.valueOf(event.getLevel()));
	} catch (Exception e) {
		e.printStackTrace();
	}
  ...
  public String getHeader()
  {
    ...
    sbuf.append("th {background: #336699; color: #FFFFFF; text-align: center;}" + Layout.LINE_SEP);
    sbuf.append("-->" + Layout.LINE_SEP);
    ...
    sbuf.append("<table cellspacing=\"0\" cellpadding=\"4\" border=\"1\" bordercolor=\"#224466\" width=\"200%\">" + Layout.LINE_SEP);
    ...
    sbuf.append("<th>文件:行号</th>" + Layout.LINE_SEP);
